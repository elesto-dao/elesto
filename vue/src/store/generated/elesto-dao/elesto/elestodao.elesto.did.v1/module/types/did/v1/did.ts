...
export interface DidMetadata {
  /** version id is the tansaction hash of the did creation */
  versionId: string;
...
export interface PublicKeyJwk {
  /** (Key ID) used to match a specific key, and for did key fragment */
  Kid: string;
  ...
  Y: string;
  /** identifies the key type (e.g. EC for elliptic curve) */
  Kty: string;
}
...
const basePublicKeyJwk: object = { Kid: "", Crv: "", X: "", Y: "", Kty: "" };

export const PublicKeyJwk = {
  encode(message: PublicKeyJwk, writer: Writer = Writer.create()): Writer {
    if (message.Kid !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): PublicKeyJwk {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...basePublicKeyJwk } as PublicKeyJwk;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): PublicKeyJwk {
    const message = { ...basePublicKeyJwk } as PublicKeyJwk;
  ...
  toJSON(message: PublicKeyJwk): unknown {
    const obj: any = {};
  ...
  fromPartial(object: DeepPartial<PublicKeyJwk>): PublicKeyJwk {
    const message = { ...basePublicKeyJwk } as PublicKeyJwk;
    if (object.Kid !== undefined && object.Kid !== null) {
