...
func TestMsgIssuePublicVerifiableCredentialRequest_ValidateBasic(t *testing.T) {
	type fields struct {
		...
		CredentialDefinitionDid string
		Signer                  string
	}
	tests := []struct {
		name    string
	...
	}{
		{
			...
			fields{
				...
				CredentialDefinitionDid: "did:cosmos:elesto:cd-0001",
				Signer:                  "elesto17t8t3t6a6vpgk69perfyq930593sa8dnfl98mr",
			},
			nil,
		},
		{
			"FAIL: empty CredentialDefinitionDid",
			fields{
				Credential: &PublicVerifiableCredential{
					Id: "https://test.xyz/credential/1",
					Context: []string{
		...
		},
		{
			...
			fields{
				...
				CredentialDefinitionDid: "not a did",
				Signer:                  "elesto17t8t3t6a6vpgk69perfyq930593sa8dnfl98mr",
			},
			errors.New("credential definition id must be a valid DID"),
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := MsgIssuePublicVerifiableCredentialRequest{
				...
				CredentialDefinitionDid: tt.fields.CredentialDefinitionDid,
				Signer:                  tt.fields.Signer,
			}
			err := m.ValidateBasic()
