...
func (k msgServer) RegisterCredentialIssuer(
	...
	if strings.HasPrefix(msg.Issuer.Did, did.DidKeyPrefix) {
		err := sdkerrors.Wrapf(did.ErrInvalidInput, "did documents having id with key format cannot be used for credential issuers %s", msg.Issuer.Did)
	...
	if !did.IsEmpty(msg.RevocationServiceURL) {
